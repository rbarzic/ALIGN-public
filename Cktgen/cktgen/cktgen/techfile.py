...
#        fp.write( f"MetalTemplateInstance template={mt.name} pgdoffset_abs=0 ogdoffset_abs={mt.stop_offset} region={':'.join( str(i) for i in bbox)}\n")
        fp.write( f"MetalTemplateInstance template={mt.name} pgdoffset_abs=0 region={':'.join( str(i) for i in bbox)}\n")
